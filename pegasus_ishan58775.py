#!/usr/bin/env python3
# Modified by: Ishan Keshav
# Pegasus v1.1 — ADB Utility Toolkit (password protected, license removed)

import os
import base64
import zlib
import sys
from getpass import getpass
from colorama import Fore, Style, init

init(autoreset=True)

# -------- Screen clear --------
os.system("cls" if os.name == "nt" else "clear")

# -------- Password prompt (exact match required) --------
PASSWORD = "FireWallBreaker"
pw = getpass(Fore.YELLOW + "Enter password to run Pegasus: ")
if pw != PASSWORD:
    print(Fore.RED + "Wrong password!")
    sys.exit(1)

# -------- SAME SECRET used in encryptor.py --------
SECRET = b"secret!"

# -------- Paste your v1.1 BLOB here (unchanged) --------
BLOB = ""

def xor_bytes(data: bytes, key: bytes) -> bytes:
    return bytes([b ^ key[i % len(key)] for i, b in enumerate(data)])

def decrypt(b64s: str) -> str:
    raw = base64.b64decode(b64s.encode())
    plain = zlib.decompress(xor_bytes(raw, SECRET))
    return plain.decode("utf-8")

# -------- Decrypt & run payload (NO LICENSE CHECK) --------
code = decrypt(BLOB)
ns = {}
exec(code, ns)      # payload must define run()
ns["run"]()         # execute payload