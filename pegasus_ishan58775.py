import os, base64, zlib, sys, hashlib, hmac
from colorama import Fore, Style, init
init(autoreset=True)

# -------- Screen clear --------
os.system("cls" if os.name == "nt" else "clear")

# -------- SAME SECRET used in encryptor.py --------
SECRET = b"secret!"

# -------- Paste your BLOB here --------
BLOB = ""

def xor_bytes(data: bytes, key: bytes) -> bytes:
    return bytes([b ^ key[i % len(key)] for i, b in enumerate(data)])

def decrypt(b64s: str) -> str:
    return zlib.decompress(xor_bytes(base64.b64decode(b64s), SECRET)).decode()

# -------- LICENSE HASH CHECK --------
LICENSE = "ishan58775-license-123"  # optional string
hash_digest = hashlib.sha256(LICENSE.encode()).hexdigest()

if hash_digest[:6] != "e99a18":  # sample condition
    print(Fore.RED + "[!] Invalid License!")
    sys.exit()

print(Fore.GREEN + "[✓] License Verified!\n")

# -------- EXECUTE PAYLOAD --------
ns = {}
exec(decrypt(BLOB), ns)
ns["run"]()
